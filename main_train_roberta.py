...
def read_m_file(text_file, valP, testP, robertaWordEmbPath):
    data_file = open(text_file, 'r')
    data_array = []
    word_id = {}
    id_word = {}
    SModel = SentenceTransformer('sentence-transformers_paraphrase-multilingual-MiniLM-L12-v2')
    for data_line in data_file.readlines():
        data_line = data_line.strip().split('\t')
        question = data_line[0].split('[')[0]+'NE'+data_line[0].split('[')[1].split(']')[1]
        data_array.append([data_line[0].split('[')[1].split(']')[0].strip(), question.strip(), data_line[1].split('|')])
        for word in question.strip().split():
            if word not in word_id :
                word_id[word] = len(word_id)+1
                id_word[len(word_id)] = SModel.encode(word, convert_to_tensor=True)

    data_file = open(valP, 'r')
    for data_line in data_file.readlines():
        data_line = data_line.strip().split('\t')
        question = data_line[0].split('[')[0]+'NE'+data_line[0].split('[')[1].split(']')[1]
        for word in question.strip().split():
    ...
    pickle.dump({"da":data_array, "wd":word_id, "iw":id_word}, open(robertaWordEmbPath+"wordEmbed.pkl","wb"))
    return data_array, word_id, id_word



def model_validation(data_path, device, model,entity_id, id_word):
    model.eval()
...
robertaWordEmbPath="./RobertaEmb/"+datasetName

if roberta_word_embbeding_exist:
    r_emb_p=pickle.load(open(robertaWordEmbPath+"/wordEmbed.pkl", "rb"))


if not os.path.exists("./Models/"+datasetName):
    os.makedirs("./Models/"+datasetName)
if not os.path.exists(embedding_folder):
    os.makedirs(embedding_folder)
if not os.path.exists(model_store_path):
    os.makedirs(model_store_path)
if not os.path.exists("./RobertaEmb/"):
    os.makedirs("./RobertaEmb/")
if not os.path.exists(robertaWordEmbPath):
    os.makedirs(robertaWordEmbPath)
...
if trainKBEmbeddingFlag:
    ...
    trainE=trainE()
    trainE.train(kbPath, kbSplit,  datasetName, embedding_folder, cuda, learning_rate, decay_rate, nb_epochs, batch_size)
...
if roberta_word_embbeding_exist:
    ...
    data, word_id, id_word = read_m_file(data_path, valid_data_path, test_data_path, robertaWordEmbPath)
...
for epoch in range(0,nb_epochs):
    ...
    for j in tqdm(range(0, len(data), batch_size)):
        ...
        loss = model(question, q_head, q_tail, ques_len, id_word, len(data_batch))
        loss.backward()
        optimizer.step()
        lossV+= loss.item()
    ...
    if epoch%10 == 0:
        model.eval()
        answers, score, _, _, _, _, _, _, _, _, _, _, _ = model_validation(valid_data_path, device, model, entity_id, id_word)
        if score > bscore:
            bscore = score
